...
for i=1:10
    ...
    % Reading Train Data
    filename = fullfile(folderpath,Train_names{i});
    delimiter = ',';
    ...
    % Training the data
    Total_data = table(dataArray{1:end-1}, 'VariableNames', {'VarName1','VarName2','VarName3','VarName4','VarName5'});
    Total_data_svm =  Total_data{1:size(Total_data,1),1:4} ;          % without last column
    result = zeros(size(Total_data,1),1);
    result(1:153,1)=1;
    % Call fitcsvm to train SVM model.
    
   svm_model =  fitcsvm(Total_data_svm,result,'standardize',true,'KernelFunction','gaussian','BoxConstraint',1,'OutlierFraction',0.10);
	...
	[prediction] = predict(svm_model,Total_Test_data_svm);
...
delete('Performance_SVM.csv');
csvwrite('Performance_SVM.csv', matrix2);
clearvars matrix2;
