...
%     final_result = mtimes(MaxMatrix, wcoeff)

% plot(wcoeff);

%  %for j = 1:1
% %     clf
%      for k = 1:20*numel(myFolder)
%          hold all;
%          scatter(final_result(k,1),final_result(k,2),'MarkerEdgeColor','red')
%         pause(0.1);
%      end
% % end
%
%
% xlabel('PCA Component 1','FontWeight','bold','FontSize',16);
% ylabel('PCA Component 2','FontWeight','bold','FontSize',16);
% title(action_name,'FontSize',16 );
% %lgd=legend('1st Instance','2nd Instance','3rd Instance' )
% %lgd.FontSize = 8;
% delete(strcat('./PCA_op/', strcat('PCA','_',action_name)))
% saveas(gcf,strcat('./PCA_op/', strcat('PCA','_',action_name)), 'jpeg')


%
% x1=1:50;
% for i=1:10
%     y1=VarMatrix(i:i,1:50);
%     plot(x1,y1)
% hold on
% end
%
